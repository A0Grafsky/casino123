...
async def append_user_to_user_tabl(message: Message, state: FSMContext):
    ...
    if nickname not in [i[0] for i in (await db.nickname_from_users())]:
        ...
        referral_id = await state.get_data()
        if referral_id['referral_id'] != str(message.from_user.id):
            async with aiosqlite.connect('userdata.db') as conn:
                await conn.execute('UPDATE users SET referral = ? WHERE user_id = ?', (referral_id, message.from_user.id))
                await conn.commit()
                get_lucky_coins = await conn.execute('SELECT Lucky FROM users WHERE user_id = ?', (referral_id))
                get_lucky_coins = await get_lucky_coins.fetchall()
                await conn.execute('UPDATE users SET Lucky = ? WHERE user_id = ?', (get_lucky_coins[0][0] + 3, referral_id))
                await conn.commit()
            await message.answer(f'Вы зашли по реферральной ссылке человека, с id {message.text.split(" ")[1]}')
        else:
